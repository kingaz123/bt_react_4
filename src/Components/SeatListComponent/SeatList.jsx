...
import SeatRow from "../SeatRowComponent/SeatRow";
...
export default class SeatList extends Component {
  ...
  showSeatList = () => {
    return this.seatList.map((seatRow, index) => {
      return <SeatRow key={index} seatRow={seatRow} />;
    });
  };
  render() {
    return (
      <div className="text-center">
        <h3 className="text-warning text-uppercase pt-4">
          Đặt vé xem phim Cyberlearn.vn
        </h3>
        <p className="text-white mt-4">Màn hình</p>
        <div className="d-flex justify-content-center">
          <p className="screen"></p>
        </div>
        <div className="pr-4">{this.showSeatList()}</div>
      </div>
    );
  }
}
